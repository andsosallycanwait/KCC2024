...
Running parameters: 
--output_dir=foodbert/data/mlm_output --model_type=bert --model_name=bert-base-cased --do_train 
--train_data_file=foodbert/data/train_instructions.txt --do_eval --eval_data_file=foodbert/data/test_instructions.txt 
--mlm --line_by_line --per_gpu_train_batch_size=16 --gradient_accumulation_steps=2 --per_gpu_eval_batch_size=16 
--save_total_limit=5 --save_steps=10000 --logging_steps=10000 --evaluate_during_training 
--model_name_or_path=foodbert/data/mlm_output/checkpoint-final
model_name_or_path can need adjustment
'''
...
def main():
    ...
    tokenizer = BertTokenizer(vocab_file='foodbert/data/bert-base-cased-vocab.txt', do_lower_case=False,
                              max_len=128, never_split=used_ingredients)  # For one sentence instruction, longer shouldn't be necessary
